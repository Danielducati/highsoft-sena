...
function formatVenta(v) {
  ...
  return {
    ...
    items:       (v.Venta_detalle ?? []).map(d => ({
      ...
      empleado:   d.usuario?.empleado?.[0]
                    ? `${d.usuario.empleado[0].nombre} ${d.usuario.empleado[0].apellido}`
                    : null,
    })),
  };
}
...
const include = {
  Cliente: true,
  Venta_detalle: {
    include: { servicio: true, usuario: { include: { empleado: true } } },
  },
};
...
const getAvailableAppointments = async () => {
  const citas = await prisma.agendamientoCita.findMany({
    ...
    include: {
      cliente:  true,
      detalles: { include: { servicio: true, usuario: true } },
    },
    orderBy: { fecha: "desc" },
  });
...
const create = async ({ tipo, clienteId, citaId, servicios, descuento, metodoPago }) => {
  return prisma.$transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      // Permite enviar empleadoId desde frontend y lo convierte al usuarioId real.
      let usuarioId = item.usuarioId ? Number(item.usuarioId) : null;
      if (!usuarioId && item.empleadoId) {
        const emp = await tx.empleado.findUnique({
          where: { id: Number(item.empleadoId) },
