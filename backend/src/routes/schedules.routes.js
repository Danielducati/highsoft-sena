...
// Agrupa horarios por empleado + semana
const map = new Map();
...
for (const h of horarios) {
    ...
    const dayIndex  = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // lunes=0 ... domingo=6
...
// POST /schedules — crear semana completa
...
// PUT /schedules/:employeeId/:weekStartDate — reemplazar semana
router.put("/:employeeId/:weekStartDate", async (req, res) => {
try {
    ...
    await prisma.$transaction(async (tx) => {
    // Eliminar horarios de esa semana para ese empleado
...
// DELETE /schedules/:employeeId/:weekStartDate
router.delete("/:employeeId/:weekStartDate", async (req, res) => {
try {
    const { employeeId, weekStartDate } = req.params;
    const monday = new Date(weekStartDate + "T00:00:00");
    const sunday = new Date(monday); sunday.setDate(monday.getDate() + 6);
